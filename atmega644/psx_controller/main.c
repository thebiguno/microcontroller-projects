...
int main (void){
	...
	while (1){
		...
		if (state & PSB_SELECT && change & PSB_SELECT) serial1_write_s("Select\n\r");
		...
		if (state & PSB_START && change & PSB_START) serial1_write_s("Start\n\r");
		...
		if (state & PSB_TRIANGLE && change & PSB_TRIANGLE) serial1_write_s("Triangle\n\r");

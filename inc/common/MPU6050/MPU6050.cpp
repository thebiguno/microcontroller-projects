...
void MPU6050::calibrate(){
	...
	for (uint16_t i = 0; i < count; i++){
		...
		totals[0] += (int16_t) ((data[0] << 8) | data[1]);
		totals[1] += (int16_t) ((data[2] << 8) | data[3]);
		totals[2] += (int16_t) ((data[4] << 8) | data[5]);
		totals[3] += (int16_t) ((data[8] << 8) | data[9]);
		totals[4] += (int16_t) ((data[10] << 8) | data[11]);
		totals[5] += (int16_t) ((data[12] << 8) | data[13]);
...
vector_t MPU6050::getAccel(){
	...
	i2c->read(MPU6050_ADDRESS, &message);				//Read 6 bytes (Accel X/Y/Z, 16 bits signed each)
 	...
 	vector_t result;
	result.x = (((int16_t) ((data[0] << 8) | data[1])) + calibration[0]) * 0.00048828125;		// 16g / 32768
	result.y = (((int16_t) ((data[2] << 8) | data[3])) + calibration[1]) * 0.00048828125;
	result.z = (((int16_t) ((data[4] << 8) | data[5])) + calibration[2]) * 0.00048828125;
 	return result;
}
...
vector_t MPU6050::getGyro(){
	...
	i2c->read(MPU6050_ADDRESS, &message);				//Read 6 bytes (Gyro X/Y/Z, 16 bits signed each)

	vector_t result;
	result.x = (((int16_t) ((data[0] << 8) | data[1])) + calibration[3]) * (0.06103515625 * M_PI / 180);		//(2000 deg/s / 32768) * PI/180.
	result.y = (((int16_t) ((data[2] << 8) | data[3])) + calibration[4]) * (0.06103515625 * M_PI / 180);
	result.z = (((int16_t) ((data[4] << 8) | data[5])) + calibration[5]) * (0.06103515625 * M_PI / 180);
	return result;
}

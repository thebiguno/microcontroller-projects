...
vector_t Mpu6050::getGyro(){
	...
	vector_t result;
	result.x = (((data[0] << 8) | data[1]) + gyroCalib[0]) * 0.00762939453125;		//250 deg/s / 32768
	result.y = (((data[2] << 8) | data[3]) + gyroCalib[1]) * 0.00762939453125;
	result.z = (((data[4] << 8) | data[5]) + gyroCalib[2]) * 0.00762939453125;
	return result;
}

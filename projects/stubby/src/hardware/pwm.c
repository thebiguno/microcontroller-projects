...
void pwm_set_phase(uint8_t index, uint32_t phase, uint8_t counter){
	pwm_set_phase_batch(index, phase, counter);
	pwm_apply_batch();
}

void pwm_set_phase_batch(uint8_t index, uint32_t phase, uint8_t counter){
	//Bounds checking
	if (index >= _count) {
		return;
	}

...
void processing_command_executor(){
	...
	if (heading_confirmation_required && debug){
		char* temp = (char*) "Start heading confirmation";
		doSendDebug(temp);
	}
	while (heading_confirmation_required){
		...
		if (debug){
			char temp[64];
			sprintf(temp, "Read heading: %1.3f", heading);
			doSendDebug(temp);
		}
		...
		if (count >= 5){
			...
			if (debug){
				char temp[64];
				sprintf(temp, "Finish heading confirmation: %1.3f", heading);
				doSendDebug(temp);
			}
	...
	if (move_required && !heading_confirmation_required){
	...
	else if (turn_required && !heading_confirmation_required && !heading_confirmation_required){
		static uint8_t step_index = 0;
		double current_heading = magnetometer_read_heading();
		...
		if (fabs(difference) <= HEADING_MARGIN_OF_ERROR){
			...
			if (debug){
				char temp[128];
				sprintf(temp, "Turning complete: current: %1.3f, desired: %1.3f, diff: %1.3f", current_heading, desired_turn_heading, difference);
				doSendDebug(temp);
			}
			...
			double velocity = fmax(0.1, fmin(desired_turn_velocity, fabs(difference)));
			...
			if ((step_index & 0x03) == 0 && debug){
				char temp[128];
				sprintf(temp, "Turning: current: %1.3f, desired: %1.3f, diff: %1.3f", current_heading, desired_turn_heading, difference);
				doSendDebug(temp);
			}

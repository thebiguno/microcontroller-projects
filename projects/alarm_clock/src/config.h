...
typedef struct config {
	uint8_t source;		//Source to play when manually turning on music.  Must be a named folder on the SD card.  Can pick from folders 01 to 08.  If bit 7 is set, we use the FM radio.
	...
	uint16_t music_fm_channel;	//FM Channel (saved automatically).

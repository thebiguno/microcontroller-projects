...
void State::poll(){
	...
	if (this->state == STATE_TIME){
		if (button.longPressEvent()){
			...
			if (this->light_brightness){
				this->light_brightness = 0;
				light_off();
			}
			else {
				this->light_brightness = 512;
				light_on();
			}
		}
		else if (encoder_movement != 0 && this->light_brightness != 0){
			this->light_brightness += encoder_movement * 20;
			if (this->light_brightness < 1) {
				this->light_brightness = 1;
			}
			else if (this->light_brightness > 1024){
				this->light_brightness = 1024;
			}
		}
	}
...
// 	else if (this->state == STATE_MENU){
// 		Channel* channel = &channels[this->scroll_channel];
// 		int8_t negate = channel->get_voltage_limit() < 0 ? -1 : 1;		//We assume that a channel will be negative for both voltage and current.  This should always hold true.int16_t get_light_brightness();

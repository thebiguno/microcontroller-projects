...
static volatile uint8_t _running = 0;							//Set to 1 when running, set to 0 when stopped.  Used to determine which output mode we are in.  **** This was added when the file was coped out of lib folder, to support DDS function generation modes. ****

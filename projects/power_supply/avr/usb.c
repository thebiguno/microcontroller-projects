		...
		switch(rx_buffer[0]){
			...
			case MESSAGE_CHANGE_SETPOINT: {
				...
				if (channel < CHANNEL_COUNT){
					...
					usb_send_values(MESSAGE_CHANGE_SETPOINT, channel, channels[channel].get_voltage_setpoint(), channels[channel].get_current_setpoint());
				}
				break;
			}
			case MESSAGE_CHANGE_SETPOINT_RAW: {
				...
				if (channel < CHANNEL_COUNT){
					...
					usb_send_values(MESSAGE_CHANGE_SETPOINT_RAW, channel, channels[channel].get_voltage_setpoint_raw(), channels[channel].get_current_setpoint_raw());
				}
				break;
			}
		}

...
namespace digitalcave {
	...
	/*
	 * A Ride object is a special type of Pad which uses two analog input channels (pedal position and cymbal
	 * strike velocity) along with a digital switch (pedal tightly closed) to play back sounds.  It is pretty
	 * much identical to the hi hat, except that instead of having a range of values for openness, it is binary
	 * (closed HH pedal is edge, open HH pedal is bell), and it does not mute the playing sounds when it 
	 * transitions from open to closed.
	 */
	class Ride : public Cymbal {
		private:
		...
		public:
			...
			Ride(uint8_t piezoIndex, uint8_t pedalMuxIndex, uint8_t switchMuxIndex, uint8_t doubleHitThreshold, double fadeGain);
